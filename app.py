...
# Cache S3 Downloads to Reduce API Calls
@st.cache_data
def fetch_s3_file(s3_url):
    try:
        bucket, key = s3_url.replace("s3://", "").split("/", 1)
...
st.markdown("""
    <style>
        ...
        .block-container { padding: 50px; }
        div[data-testid="stTextInput"] input { background-color: #222 !important; color: white !important; }
        div[data-testid="stButton"] button { background-color: #444 !important; color: white !important; }
    </style>
""", unsafe_allow_html=True)

st.markdown("### Porosity Defect Inspection")
col1, col2, col3 = st.columns([3, 1, 1])
...
if block_number:
    ...
    if not items:
        ...
        csv_data = fetch_s3_file(report_url) if report_url else None
        image_data = fetch_s3_file(localized_url) if localized_url else None
        mask_data = fetch_s3_file(mask_url) if mask_url else None
        ...
        with col4:
            ...
            try:
                if image_data:
                    ...
                    if show_mask and mask_data:
                        mask_image = Image.open(BytesIO(mask_data)).convert("RGBA")
                        mask_image = mask_image.resize(base_image.size, Image.BILINEAR)

                        # Use Alpha < 0.5 for Better Overlay Performance
                        blended_image = Image.blend(base_image, mask_image, alpha=0.4)
                        image_zoom(blended_image, mode="scroll", size=(700, 500), zoom_factor=8.0, increment=0.8)
                    else:
                        image_zoom(base_image, mode="scroll", size=(700, 500), zoom_factor=8.0, increment=0.8)
                else:
                    st.warning("Localized image not found.")
            except Exception as e:
                st.error(f"Error loading images: {e}")
